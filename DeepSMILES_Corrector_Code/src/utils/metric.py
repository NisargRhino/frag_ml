...
def molecule_reconstruction(array, TRG, reverse: bool, outputs):
    ...
    for i in range(0, batch_size):
        ...
        trg_tokens = [trg_field.vocab.itos[int(t)] for t in sequence]
        rev_tokens = list(itertools.takewhile(lambda x: x != "<eos>", trg_tokens))
        if reverse:
            rev_tokens = rev_tokens[::-1]
        smiles = "".join(rev_tokens)
...
def calc_complexity(array, TRG, reverse=False, valids=None):
            ...
            if self.loader_valid is not None:
                ...
                if reconstruction_error < best_error:
                    ...
                    if epoch - last_save > 10:
                        ...
                        print(info)
                        break
            elif self.loader_valid is None:
                torch.save(self.state_dict(), f"{self.out}.pkg")
            end_time = time.time()
...
class Convo(object):  # <- Ensure it's a new-style class
    def __init__(self, loader_train=None, out=None):
    ...
    def train_model(self):
        ...
        for epoch in range(self.epochs):
            ...
            for i, batch in enumerate(self.loader_train):
                ...
                trg = batch.trg
                src = batch.src
                src_tensor, src_lengths = src
                output, attention = self(src_tensor, trg[:, :-1])
                # feed the source and target into def forward to get the output
                # Xuhan uses forward for this, with istrain = true
                output_dim = output.shape[-1]
                # changed
            ...
            if self.loader_valid is not None:
                ...
                if reconstruction_error < best_error:
                    ...
                    if epoch - last_save >= 10 and best_error != 1:
                        ...
                        print(info)
                        break
            elif 'error' in locals() and error < best_error:
                torch.save(self.state_dict(), f"{self.out}.pkg")
                best_error = error
            end_time = time.time()
            epoch_mins, epoch_secs = epoch_time(start_time, end_time)
    ...
    def evaluate(self, return_output):
        ...
        with torch.no_grad():
            for _, batch in enumerate(self.loader_valid):
                ...
                if isinstance(src, tuple):  # Ensure proper unpacking
                    src, _ = src
                ...
                output_dim = output.shape[-1]
                output = output.contiguous().view(-1, output_dim)
                trg = trg[:, 1:].contiguous().view(-1)  # <- Fixed line
