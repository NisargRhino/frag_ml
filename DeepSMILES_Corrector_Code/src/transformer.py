...
from src.utils.metric import Convo
...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        V = V.view(batch_size, v_len, self.n_heads, self.head_dim).transpose(1, 2)
        ...
        x = x.transpose(1, 2).contiguous().view(batch_size, q_len, self.hid_dim)
        x = self.fc_o(x)
...
class Seq2Seq(nn.Module, Convo):
    def __init__(
        ...
        Convo.__init__(self, loader_train=loader_train, out=out)
    ...
    def forward(self, src, trg):
        # src = [batch size, src len]
        # trg = [batch size, trg len]
        if isinstance(src, tuple):  # unpack (src_tensor, src_lengths)
            src, _ = src
