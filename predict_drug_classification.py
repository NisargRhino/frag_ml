...
model = RobertaForMaskedLM.from_pretrained('./model_drug_classification')#enter path of the model from train_model.py
tokenizer = RobertaTokenizer.from_pretrained('./tokenizer_drug_classification')#enter path of the tokenizer from train_model.py
model.eval()

# Load the unique SMILES dataset
unique_tags_df = pd.read_csv('./drug_classification_data_df.csv')# enter the path of unique_smile5.csv
...
def find_closest_valid_smiles(predicted_tags, unique_tags_list):
    closest_tags = None
    highest_similarity = -1
    for tags in unique_tags_list:
        similarity = string_similarity(predicted_tags, tags)
